...
第一个Lin合成事件的二维风场使用与TCR完全相同的官方C15逐时径向廓线。对北半球事件，按球面大圆距离与初始方位将C15风速投影为逆时针切向矢量，再按Lin & Chavas (2012, DOI `10.1029/2011JD017126`)叠加空间均一的地表背景风：平移矢量乘`0.55`并逆时针旋转`20°`。不增加径向背景风衰减、外缘taper、经验尾部、入流角或额外`0.85`地表折减。
