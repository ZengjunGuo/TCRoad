#!/usr/bin/env python3
"""Mechanical Python 3 compatibility adapter for official CLE15 v1.0.

Source: vendor/CLE15/10.4231_CZ4P-D448/v1.0/original/CLE15_2020-06-23.py

Only language/runtime compatibility edits are permitted here.  Physical equations,
default constants, grids, convergence thresholds, and iteration logic must remain
identical to the frozen official source.  See TRANSFORM_LOG.md for the complete
edit contract and validation status.
"""
...
def E04_outerwind_r0input_nondim_MM0(r0,fcor,Cdvary,C_d,w_cool,Nr):
    ...
    MMfracM0 = float('NaN')*np.zeros(rrfracr0.size)  #[] M/M0 vector initialized to 1 (M/M0 = 1 at r/r0=1)
    MMfracM0[-1] = 1
    ...
    while soln_converged==0:
        ...
        while np.abs(drmaxr0)>=drmaxr0_thresh:
            ...
            if not np.isnan(np.max(VV_ER11)):    #ER11_radprof converged
                ...
                if intersection.is_empty:    #no intersections -- rmaxr0 too small
...
def ER11E04_nondim_rmaxinput(Vmax,rmax,fcor,Cdvary,C_d,w_cool,CkCdvary,CkCd,eye_adj,alpha_eye):
    ...
    if soln_converged == 1: 
       ...
       while np.abs(drmaxr0)>=drmaxr0_thresh:
           ...
           l2 = LineString(list(zip(rrfracr0_ER11,MMfracM0_ER11)))
           intersection = l1.intersection(l2)
           if intersection.is_empty:   ##no intersections r0 too large --> rmaxr0 too small 
               drmaxr0 = np.abs(drmaxr0)/2
           ...
           rmaxr0 = rmaxr0_new    #this is the final one
           rmaxr0_new = rmaxr0_new + drmaxr0
       # MATLAB-authoritative correction: the official MATLAB implementation
       # always builds from the most recently stored merge point.  The separate
       # official Python file's final-intersection fallback is therefore absent.
       ## Calculate some things
       M0 = .5*fcor*r0**2
       Mm = .5*fcor*rmax**2 + rmax*Vmax
       MmM0 = Mm/M0
